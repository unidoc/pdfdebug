...
func TestStream_FixturesParseThroughOpenPath(t *testing.T) {
	...
	for _, name := range []string{"leading-plus.pdf", "comment-and-dangling.pdf"} {
		...
		if ec != 0 {
			t.Fatalf("[P0] 14.1-INTG-000: fixture %q rejected by the existing open path (exit %d): %s", name, ec, stderr)
		}
	}
}
...
func TestStream_LeadingPlusJSON(t *testing.T) {
	...
	if ec != 0 {
		t.Fatalf("[P1] 14.1-INTG-001: --json must exit 0, got %d\nstdout: %s\nstderr: %s", ec, stdout, stderr)
	}

	var res streamJSON
	if err := json.Unmarshal([]byte(stdout), &res); err != nil {
		t.Fatalf("[P1] 14.1-INTG-001: failed to parse --json output: %v\nraw: %s", err, stdout)
	}
	...
	for _, tk := range res.Tokenized {
		if tk.Value == "+5" {
			plusCount++
			if tk.Type != "number" {
				t.Errorf("[P1] 14.1-INTG-001: token %q has Type %q, want \"number\" (leading '+' mislabeled)", tk.Value, tk.Type)
			}
		}
	}
	if plusCount != 2 {
		t.Errorf("[P1] 14.1-INTG-001: found %d \"+5\" tokens, want 2 (content stream is \"+5 0 0 +5 0 0 cm\")", plusCount)
	}
	...
	for _, fl := range res.Formatted {
		if fl.Operator == "+5" {
			t.Errorf("[P1] 14.1-INTG-001: a formatted operation is named operator %q; signed operands must not flush as their own operation", fl.Operator)
		}
		if fl.Operator == "cm" {
			sawCM = true
		}
	...
	if !sawCM {
		t.Errorf("[P1] 14.1-INTG-001: no formatted operation with operator \"cm\"; the six signed operands should group under the single cm operator")
	}
}
...
func TestStream_CommentAndDanglingOps(t *testing.T) {
	...
	if ec != 0 {
		t.Fatalf("[P1] 14.1-INTG-002: --ops must exit 0, got %d\nstdout: %s\nstderr: %s", ec, stdout, stderr)
	}
	...
	for _, line := range strings.Split(strings.TrimSpace(stdout), "\n") {
		...
		if err := json.Unmarshal([]byte(line), &rec); err != nil {
			t.Fatalf("[P1] 14.1-INTG-002: NDJSON line is not valid JSON: %v\nline: %s", err, line)
		}
		if rec.Op == "" {
			emptyOps++
			t.Errorf("[P1] 14.1-INTG-002: NDJSON record with empty op (phantom no-op record breaches the one-object-per-operator contract): %s", line)
		}
		if rec.Op == "cm" {
			sawCM = true
		}
	...
	if emptyOps != 0 {
		t.Errorf("[P1] 14.1-INTG-002: %d record(s) with op == \"\"; want 0 (comment and dangling-operand lines must not emit)", emptyOps)
	}
	if !sawCM {
		t.Errorf("[P1] 14.1-INTG-002: no record with op == \"cm\"; the real operator must still emit (got %d records)", records)
	}
}
...
func TestStream_LeadingPlusOps(t *testing.T) {
	...
	if ec != 0 {
		t.Fatalf("[P2] 14.1-INTG-003: --ops must exit 0, got %d\nstdout: %s\nstderr: %s", ec, stdout, stderr)
	}
	...
	for _, line := range strings.Split(strings.TrimSpace(stdout), "\n") {
		...
		var rec opsRecord
		if err := json.Unmarshal([]byte(line), &rec); err != nil {
			t.Fatalf("[P2] 14.1-INTG-003: NDJSON line is not valid JSON: %v\nline: %s", err, line)
		}
		// A "+5" must never surface as an operator record: it is a numeric operand.
		if rec.Op == "+5" {
			plusRecords++
			t.Errorf("[P2] 14.1-INTG-003: NDJSON record named op == %q; a signed operand must not flush as its own operation: %s", rec.Op, line)
		}
		if rec.Op == "cm" {
			cmRecords++
			cmParams = rec.Params
	...
	if plusRecords != 0 {
		t.Errorf("[P2] 14.1-INTG-003: %d record(s) named op == \"+5\"; want 0", plusRecords)
	}
	if cmRecords != 1 {
		t.Fatalf("[P2] 14.1-INTG-003: found %d \"cm\" records, want exactly 1 (the six signed operands group under a single cm)", cmRecords)
	}
	...
	if len(cmParams) != len(want) {
		t.Fatalf("[P2] 14.1-INTG-003: cm record has %d params %v, want %d %v", len(cmParams), cmParams, len(want), want)
	}
	for i := range want {
		if cmParams[i] != want[i] {
			t.Errorf("[P2] 14.1-INTG-003: cm param[%d] = %q, want %q (full: %v)", i, cmParams[i], want[i], cmParams)
		}
	}
}
