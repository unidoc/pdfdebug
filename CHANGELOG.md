...
- The CLI now says when a newer release exists: a small ASCII box at the end of stderr, shown once per version with one reminder a week later. It only appears when stdout and stderr are both terminals, and never under `CI`, `--json`, the other machine formats, or when `PDFDEBUG_NO_UPDATE_CHECK` or `NO_UPDATE_NOTIFIER` is set; it never changes an exit code. `--version` checks live and says whether an update is available, the build is current, or the check failed. The answer comes from a per-user cache (`pdfdebug/updatecheck.json` under the platform cache directory) that the desktop app now shares: a check by either one serves both for 24 hours, the app's launch check skips the network when a check succeeded in the last 24 hours and found nothing newer, and "Check for Updates..." still goes live. An ordinary command only refreshes a stale cache in an interactive session, with one request bounded at 1.5 seconds
