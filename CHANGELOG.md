...
- Image sample interpretation: `dump image` and the GUI Image Metadata panel now report the `/Decode` array, `/ImageMask`, the image `/SMask` reference and the Adobe APP14 transform, plus one verdict joining the array and the marker into a single answer about whether the samples are read inverted. The two switches live in different layers - `/Decode` in the PDF, the Adobe record inside the JPEG - and the outcome depends on the combination, which is why there is one verdict rather than two rows. The marker only inverts a four-component stream, so below four components the verdict comes from `/Decode` alone and the transform is shown as evidence beside it. The marker is reported either way, and the marker walk reads no pixel: it hops segment to segment and stops at SOS. `dump image` also prints the stored and decoded sizes it already knew and had been dropping
