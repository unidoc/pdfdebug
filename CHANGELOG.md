...
- `dump bytes`, `dump xref`, `dump objects` and `dump signatures` now reject a second positional argument with the usage line and exit 1 instead of dropping it. Go's flag package stops parsing at the first non-flag argument, so `dump bytes file.pdf --json` used to stream the raw document with exit 0 while the caller waited for the JSON wrapper. Write flags before the file
